...
- **三种部署方式**：
  ...
  - PaaS + FRP 反向 TCP 隧道镜像（`ghcr.io/x-dora/rw-node:latest-paas-frp`，用于 PaaS 不能公开自定义 TCP 入站端口的场景，内置 Caddy HTTP 前置）
...
- `docker/Dockerfile.paas-frp` — PaaS 反向 TCP 隧道版镜像（内置 frpc + Caddy，发布为 `:latest-paas-frp` / `:<version>-paas-frp`）
- `docker/docker-entrypoint.sh` / `docker-entrypoint.official.sh` — 标准容器入口脚本
- `docker/docker-entrypoint.paas-frp.sh` — PaaS FRP 入口脚本，先启动 Caddy HTTP 前置，再启动 rw-node、生成 `frpc.toml`、等待本地 HTTPS 就绪后启动 frpc
...
- PaaS FRP 版默认启动 Caddy HTTP 前置用于 PaaS HTTP/HTTPS 回源场景：`${PORT:-3000}` → `/xh-*`（以 `/xh-` 开头）→ `127.0.0.1:8080`，`${PORT:-3000}` → `/ws-*`（以 `/ws-` 开头）→ `127.0.0.1:8880`；Caddy 到 Xray 一律使用明文 HTTP
