...
- `docker/Dockerfile.paas` — PaaS HTTPS 直连镜像（内置 Caddy HTTP 前置，发布为 `:latest-paas` / `:<version>-paas`）
...
- `docker/docker-entrypoint.paas.sh` / `docker-entrypoint.go-paas.sh` — PaaS 入口脚本，先启动 Caddy HTTP 前置再启动 rw-node
- `docker/paas-caddy-front.sh` — PaaS Caddy 前置共享脚本，生成路由配置并加载静态伪装页面
...
- PaaS 版默认启动 Caddy HTTP 前置用于 PaaS HTTP/HTTPS 回源场景：`${PORT:-3000}` → `/xh-*`（以 `/xh-` 开头）→ `127.0.0.1:8080`，`${PORT:-3000}` → `/ws-*`（以 `/ws-` 开头）→ `127.0.0.1:8880`；Caddy 到 Xray 一律使用明文 HTTP。`/node/*` 和 `/vision/*` 会转发到 `127.0.0.1:NODE_PORT` 的 HTTPS API，Caddy 使用 `tls_insecure_skip_verify` 不校验节点自签证书；其他路径返回静态伪装页面
