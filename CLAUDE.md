...
- **三种部署方式**：
  - Docker 镜像（`ghcr.io/x-dora/rw-node:latest` Go 实现 PaaS / `:latest-lite` JS 轻量版 / `:latest-official` 官方兼容版）
...
- `docker/Dockerfile` — 轻量版镜像（Go supervisord，无 Python，~380MB，发布为 `:latest-lite` / `:<version>-lite`）
- `docker/Dockerfile.official` — 官方兼容版镜像（Python supervisord）
- `docker/Dockerfile.go-paas` — Go 实现 PaaS HTTPS 直连镜像（发布为 `:latest` / `:latest-go-paas` / `:<go-version>` / `:<go-version>-go-paas`）
- `docker/docker-entrypoint.sh` / `docker-entrypoint.official.sh` — 标准容器入口脚本
- `docker/docker-entrypoint.go-paas.sh` — PaaS 入口脚本，先启动 Caddy Layer 4 前置再启动 rw-node
