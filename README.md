...
| 场景 | 推荐方式 | 说明 |
|------|----------|------|
| 常规 Docker / Compose | `ghcr.io/x-dora/rw-node:latest` | 轻量版，Go Supervisord，无 Python |
| 需要贴近官方镜像行为 | `ghcr.io/x-dora/rw-node:latest-official` | 官方兼容版，保留 Python Supervisord |
| Render / Koyeb / Railway / Fly.io 等 PaaS | `ghcr.io/x-dora/rw-node:latest-paas` | 内置 HAProxy HTTP 前置，支持 HTTPS 域名直连 |
| 希望更小体积、少运行时依赖 | `ghcr.io/x-dora/rw-node:latest-go-paas` 或 `install.sh --impl go` | 非官方 Go 实现，版本和行为不完全绑定上游 JS 实现 |
| 没有 Docker 的 VPS / 容器 | `scripts/install.sh` | 自动安装 Node.js、Xray-core、Supervisord 和服务脚本 |

> 需要严格兼容官方 Remnawave Node 行为时，优先使用 JS 兼容实现：`latest`、`latest-official`、`latest-paas` 或默认的一键安装脚本。
...
| 标签 | 描述 |
|------|------|
| `ghcr.io/x-dora/rw-node:latest` | 轻量版，Go Supervisord，无 Python |
| `ghcr.io/x-dora/rw-node:latest-official` | 官方兼容版，Python Supervisord |
| `ghcr.io/x-dora/rw-node:latest-paas` | PaaS HTTPS 直连版，内置 HAProxy HTTP 前置 |
| `ghcr.io/x-dora/rw-node:latest-go-paas` | 非官方 Go 实现 PaaS 版 |

Release workflow 也会发布固定版本标签：

- `ghcr.io/x-dora/rw-node:<version>`
- `ghcr.io/x-dora/rw-node:<version>-official`
- `ghcr.io/x-dora/rw-node:<version>-paas`
- `ghcr.io/x-dora/rw-node:<rw-node-go-version>-go-paas`
...
PaaS 镜像还支持 `PORT`、`HTTP_FRONT_ENABLED`、`HTTP_FRONT_PORT`、`XHTTP_UPSTREAM_PORT`、`WS_UPSTREAM_PORT`、`RW_NODE_APP_DIR` 等变量，见 [PaaS HTTPS 直连文档](docs/paas.md)。
...
- `latest-go-paas` 和 `install.sh --impl go` 使用非官方 [x-dora/rw-node-go](https://github.com/x-dora/rw-node-go) 实现，版本号跟随该项目 release，不跟随 `remnawave/node` 的上游版本号。
