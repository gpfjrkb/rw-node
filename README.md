...
| Render / Koyeb / Railway / Fly.io 等 PaaS（JS 实现） | `ghcr.io/x-dora/rw-node:latest-paas` | JS 实现 PaaS 版,内置 Caddy HTTP 前置 |
...
> 需要严格兼容官方 Remnawave Node 行为时,优先使用 JS 兼容实现:`latest-lite`、`latest-official`、`latest-paas` 或默认的一键安装脚本。`latest` 使用非官方 Go 实现。
...
| `ghcr.io/x-dora/rw-node:latest-paas` | JS 实现 PaaS HTTPS 直连版,内置 Caddy HTTP 前置 |
