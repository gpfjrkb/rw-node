...
启动器会让 `cloudflared` 默认通过 `1.1.1.1:53` 和 `1.0.0.1:53` 解析 Cloudflare Tunnel 的 SRV 记录，并使用 `--edge-ip-version auto` 自动选择 Cloudflare edge IP 版本。如果运行环境禁止访问外部 DNS resolver，`cloudflared` 仍可能退出并输出 edge discovery 错误。启动器会记录 `cloudflared exited; continuing without Cloudflare Tunnel`，并保持 Caddy 与 `rw-node-go` 继续运行。
